...
processa_info_item_contrato <- function(anos = c(2017, 2018, 2019)) {
  ...
  contratos_merenda <- processa_info_contratos(anos) %>% 
    select(id_orgao, nr_licitacao, ano_licitacao, nr_contrato, ano_contrato)
  ...
  info_item_contrato <- itens_contrato %>% 
    ...
    distinct(NR_LICITACAO, ANO_LICITACAO, CD_TIPO_MODALIDADE, NR_CONTRATO, 
    ...
    clean_names() %>%
    mutate(id_estado = "43") %>% 
    select(id_estado, cd_orgao, nr_lote, nr_licitacao, ano_licitacao, nr_contrato, 
           ano_contrato, nr_item, qt_itens_contrato, vl_item_contrato, vl_total_item_contrato)
