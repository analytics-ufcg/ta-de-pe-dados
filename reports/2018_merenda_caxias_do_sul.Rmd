...
O município de Caxias do Sul teve abertura de dois editais durante o ano de 2018 para fornecimento de gêneros alimentícios e, antes de analisar os valores das licitações, contratos e afins, um passo interessante é rever os editais de chamada pública, todos os editais e documentos estão [aqui](http://www1.tce.rs.gov.br/aplicprod/f?p=50500:1:::NO:::).  
...
[Edital - Janeiro](https://bit.ly/2Z6PCdP)  
[Edital - Julho](https://bit.ly/2S7xcbB)  


### Licitações

#### Quantas licitações Caxias do Sul fez no ano de 2018 comparado com o restante dos municípios?
...
#### Qual o valor dessas licitações?
...
nmr_estudantes <- censo_escolar_cx %>%
  filter(esfera != "Estadual Urbana", esfera != "Estadual Rural", esfera != "Estadual e Municipal") %>% 
  group_by(municipio) %>% 
...
gasto_licitacoes %>% ggplot(aes(x=qtd_estudantes/1e3, y=media_gasto/1e3)) +
  ...
  geom_label(aes(x=28, y=1555, label="Caxias do Sul"), color="#773bb0")
...
nmr_estudantes <- nmr_estudantes %>% filter(municipio == "CAXIAS DO SUL")
