...
#' @example empenhos_filtrados <- read_empenhos_federais_covid('postgres', 'postgres', 'tanamesa', 5432, 'secret')
read_empenhos_federais_covid <- function(host, user, database, port, password) {
  ...
  res <- DBI::dbGetQuery(
    ...
    str_glue(
      ...
      "AND codigo_modalidade_aplicacao IN ('90', '60', '50')"
    )
  )
