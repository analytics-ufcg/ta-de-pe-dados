...
itens_media_rs <- itens %>% 
  ...
  select(-produto) %>% 
  group_by(nome_item) %>% 
  summarise(media_produto = format(round(mean(VL_UNITARIO_ESTIMADO)), nsmall=2))

itens_tab <- left_join(itens_cx_cpp_tb, itens_media_rs, by = "nome_item") %>% 
  group_by(nome_item) %>% 
  summarise(dif_preco = sum(abs(media_produto - VL_UNITARIO_ESTIMADO) < 0.1))
...
itens_tab %>% 
  ...
  select(nome_item, QT_ITENS, SG_UNIDADE_MEDIDA, VL_UNITARIO_ESTIMADO, media_produto, VL_TOTAL_ESTIMADO) %>% 
  datatable(class = 'cell-border stripe',
            filter = 'top',
            rownames = FALSE, 
            options = list(pageLength = 10,
                           dom = 'ftp'),
            colnames = c("Nome", "Quantidade", "Unid. de medida", "Valor unid. estimado", "Preço médio - estado", "Valor tot. estimado"))
```
...
contratos_cx_cpp %>% 
  ...
  select(NR_DOCUMENTO, DT_INICIO_VIGENCIA, DT_FINAL_VIGENCIA, DT_ASSINATURA, VL_CONTRATO) %>% 
  datatable(class = 'cell-border stripe',
            filter = 'top',
            rownames = FALSE, 
            options = list(pageLength = 10,
                           dom = 'ftp'),
            colnames = c("CNPJ", "Início Vigência", "Fim da vigência", "Data de assinatura", "Valor do contrato"))
```
...
```{r}
alteracao_cx_cpp %>% 
  ggplot(aes(x = id, y = VL_ACRESCIMO)) +
  geom_point()
```
